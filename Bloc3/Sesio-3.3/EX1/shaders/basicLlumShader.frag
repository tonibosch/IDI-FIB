...
uniform vec3 posFCamera;
uniform vec3 colFCamera;
...
{	
    ...
    vec4 pfSCOE = view * vec4(posFocus,1.0);  
	  vec3 LE = pfSCOE.xyz-vertexSCO2.xyz;
    LE = normalize(LE);

    vec4 pfSCOC = vec4(posFCamera,1.0);  
    wec3 LC = pfSCOC.xyz - vertexSCO2.xyz;
    LC = normalize(LC);

    vec3 fcolor = Ambient() + Difus(NormSCO,LE,colorFocus) + Especular(NormSCO, LE, vertexSCO2, colorFocus);
    fcolor += Difus(NormSCO,LC,colFCamera) + Especular(NormSCO, LC, vertexSCO2, colFCamera);
